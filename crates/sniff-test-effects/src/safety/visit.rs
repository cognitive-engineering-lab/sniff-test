...
//! the corresponding visitor arms so toolchain-bump diffs stay mechanical.
//! Unsafe blocks anchor source-level effect groups. Non-call operations in
//! compiler-generated `BuiltinUnsafe` blocks are excluded, while calls retain
//! an explicit suppression fact because their unsafety is compiler-owned.
//!
...
/// safety grouping.
#[must_use]
pub fn call_identity_def_id(tcx: TyCtxt<'_>, def_id: DefId) -> DefId {
...
impl<'a, 'tcx> UnsafeOpVisitor<'a, 'tcx> {
    ...
    /// Records the declared trait method as a contract fallback independently
    /// from whether rustc resolved a concrete implementation.
    fn declaration_call_callee(&self, def_id: DefId, identity: DefId) -> Option<DefId> {
        (identity != def_id || self.tcx.trait_of_assoc(def_id).is_some()).then_some(identity)
    }

    fn applicable_marker_spans(&self, span: Span) -> Vec<Span> {
        let mut spans = vec![span];
        if let Some(scope) = self.safety_scopes.last() {
            spans.push(scope.marker_span);
        }
        spans
    }
    ...
    fn check_call(&mut self, expr: &'a Expr<'tcx>, fun: ExprId) {
        ...
        if sig.safety().is_unsafe() || matches!(fn_ty.kind(), ty::FnDef(..)) {
            let (callee, declaration_callee) = match fn_ty.kind() {
                ty::FnDef(def_id, _) => {
                    let identity = call_identity_def_id(self.tcx, *def_id);
                    let declaration_callee = self.declaration_call_callee(*def_id, identity);
                    (Some(identity), declaration_callee)
                }
                _ => (None, None),
            };
            self.record_call(callee, declaration_callee, expr.span);
        }
    }
