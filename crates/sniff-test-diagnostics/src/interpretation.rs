...
use sniff_test_core::report_roots::ReportRoot;
use sniff_test_core::workspace::ArtifactAnalysisGraph;
...
/// Convert core interpretations into findings with source locations.
///
/// # Errors
///
/// Returns an error if workspace facts cannot be traced or interpreted.
pub fn interpret_workspace<'tcx>(
    tcx: TyCtxt<'tcx>,
    local: &ArtifactFacts,
    ...
    let roots = report_roots
        ...
        .collect::<Vec<_>>();
    let crate_dependencies = sniff_test_core::compiler::dependencies::load_crate_dependencies(
        tcx,
        local,
        dependencies,
        metadata_loader,
    ...
    match &finding.kind {
        ...
        InterpretedFindingKind::Invocation { operation } => {
            let subject = target.map_or_else(
                || format!("{effect_name} invocation"),
                |target| format!("{effect_name} invocation to `{target}`"),
            );
            source_marker_description(
                class(Some(operation.as_str().to_owned())),
                &subject,
        ...
        InterpretedFindingKind::UnresolvedCallTarget { site } => {
            ...
            let subject = format!("{effect_name} effect coverage is incomplete for {boundary}");
            (
                class(None),
                subject.clone(),
                format!(
...
const fn unverified_reason(
    reason: sniff_test_core::artifact::UnverifiedMarkerProbeReason,
) -> &'static str {
    match reason {
        sniff_test_core::artifact::UnverifiedMarkerProbeReason::NoUsableSourceSpan => {
            "no usable span was available for marker association"
        }
        sniff_test_core::artifact::UnverifiedMarkerProbeReason::SourceUnavailable => {
...
mod tests {
    ...
    use sniff_test_core::report_roots::ReportRootKind;
    ...
    fn panic_contracts_keep_documented_obligation_finding_kind() {
        ...
        let finding = InterpretedFinding {
            effect: sniff_test_core::effects::EffectMetadata::of::<Panic>(),
