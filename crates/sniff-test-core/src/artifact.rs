...
pub use crate::namespace::{StableDefPathHash, StableInstanceHash};
...
impl ArtifactFacts {
    ...
    #[must_use]
    pub fn function_body(&self, function: FunctionId) -> Option<&FunctionFact> {
    ...
    #[must_use]
    pub fn defining_function_body(&self, function: FunctionId) -> Option<&FunctionFact> {
    ...
    #[must_use]
    pub fn definition_namespace_index(&self) -> DefinitionNamespaceIndex {
    ...
    #[must_use]
    pub fn source_marker_evidence_state(
        &self,
        function: FunctionId,
...
pub struct DefinitionNamespaceIndex {
    candidates: BTreeMap<StableDefPathHash, Vec<String>>,
}

impl DefinitionNamespaceIndex {
    #[must_use]
    pub fn candidates(&self, function: FunctionId) -> &[String] {
        self.candidates
            .get(&function.def_path_hash)
            .map_or(&[], Vec::as_slice)
    }

    pub fn iter(&self) -> impl Iterator<Item = (&StableDefPathHash, &[String])> + '_ {
        self.candidates
            .iter()
...
#[must_use]
pub fn same_call_source_site(left: &CallFact, right: &CallFact) -> bool {
...
pub struct FunctionId {
    pub def_path_hash: StableDefPathHash,
    pub instance_hash: Option<StableInstanceHash>,
}

impl FunctionId {
    #[must_use]
    pub const fn generic(def_path_hash: StableDefPathHash) -> Self {
        Self {
            def_path_hash,
            instance_hash: None,
        }
    ...
    pub fn resolution_candidates(self) -> impl Iterator<Item = Self> {
...
pub struct FunctionFact {
    ...
    pub provenance: FunctionFactProvenance,
    pub display_path: String,
    pub attributes: FunctionAttributesFact,
    ...
    pub contract_declaration: Option<FunctionTargetFact>,
    pub source_range: Option<SourceRangeFact>,
    pub calls: Vec<CallFact>,
    pub effects: Vec<EffectFact>,
    pub markers: Vec<AnnotationFact>,
    ...
    pub unverified_marker_probes: Vec<UnverifiedMarkerProbeFact>,
}

impl FunctionFact {
    #[allow(
        clippy::needless_pass_by_value,
        reason = "fact keys are owned values throughout artifact lookup"
    )]
    #[must_use]
    pub fn marker_evidence_state(
        &self,
        kind: AnnotationFactKind,
        target: AnnotationTargetFact,
        probing: AnnotationProbingFact,
...
pub enum FunctionFactProvenance {
    DefiningArtifact,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct FunctionAttributesFact {
    pub is_unsafe: bool,
    ...
    pub is_foreign: bool,
    pub namespace_candidates: Vec<String>,
}
...
pub struct CallFact {
    ...
    pub effect_group: Option<EffectGroupId>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub invocation_effects: Vec<InvocationEffectFact>,
    ...
    pub expanded_range: Option<SourceRangeFact>,
    /// Ordered outermost-to-innermost macro expansions that produced this
    /// semantic call.
    pub macro_expansions: Vec<MacroExpansionFact>,
    pub callee_range: Option<SourceRangeFact>,
    ...
    pub indirect_kind: Option<IndirectCallKindFact>,
    ...
    /// traversal.
    pub declaration_target: Option<FunctionTargetFact>,
    pub target: CallTargetFact,
}

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct InvocationEffectFact {
    pub effect: EffectKey,
    pub kind: EffectKind,
    pub effect_group: Option<EffectGroupId>,
    pub requires_documented_obligation: bool,
}

/// One macro definition and source invocation on the path to a semantic fact.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct MacroExpansionFact {
    pub macro_def: StableDefPathHash,
    pub display_path: String,
    pub source_range: Option<SourceRangeFact>,
}

#[must_use]
pub fn same_macro_provenance(left: &[MacroExpansionFact], right: &[MacroExpansionFact]) -> bool {
    left.len() == right.len()
...
impl CallTargetFact {
    ...
    #[must_use]
    pub fn function_target(&self) -> Option<&FunctionTargetFact> {
        match self {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct FunctionTargetFact {
    pub function: FunctionId,
    pub display_path: String,
    pub attributes: FunctionAttributesFact,
    pub contracts: FunctionContractsFact,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct FunctionContractsFact {
    pub effects: Vec<EffectContractFact>,
}

impl FunctionContractsFact {
    #[must_use]
    pub fn get(&self, effect: &EffectKey) -> Option<&ContractFact> {
        self.effects
            .iter()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct EffectContractFact {
    pub effect: EffectKey,
    pub contract: ContractFact,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct ContractRequirementFact {
    pub name: String,
...
pub struct AnnotationFact {
    ...
    pub source_range: Option<SourceRangeFact>,
    pub target: AnnotationTargetFact,
    pub applicable_probing: Vec<AnnotationProbingFact>,
    pub satisfactions: Vec<AnnotationSatisfactionFact>,
    pub requirements: Vec<ContractRequirementFact>,
}

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct AnnotationFactKind {
    pub effect: EffectKey,
...
impl AnnotationFactKind {
    #[must_use]
    pub const fn new(effect: EffectKey, role: AnnotationRole) -> Self {
        Self { effect, role }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum AnnotationProbingFact {
    SourceCallsite,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum UnverifiedMarkerProbeReason {
    NoUsableSourceSpan,
    SourceUnavailable,
}
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct UnverifiedMarkerProbeFact {
    pub kind: AnnotationFactKind,
    pub target: AnnotationTargetFact,
    pub probing: AnnotationProbingFact,
    pub reason: UnverifiedMarkerProbeReason,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct AnnotationSatisfactionFact {
    pub requirement: Option<String>,
    pub reason: String,
    pub structural_path: Option<Vec<usize>>,
}
