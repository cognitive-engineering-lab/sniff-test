...
#[allow(
    ...
    reason = "rustc extraction context is passed explicitly"
)]
...
#[allow(
    clippy::too_many_arguments,
    reason = "rustc edge context is passed explicitly"
)]
fn collect_edge<'tcx>(
    tcx: TyCtxt<'tcx>,
...
#[allow(
    ...
    reason = "the optional effect key preserves the shared collector interface"
)]
fn collect_mir_effect(
    effect: EffectKey,
    kind: EffectKind,
    body: &mut PendingBody,
    call_key: &str,
    ...
    let effect_key = format!("effect:{}:{}:{call_key}", effect.as_str(), kind.as_str());
...
fn attach_preliminary_operations(
    ...
    for (ordinal, registered) in facts.into_iter().enumerate() {
        ...
        for body_id in body_ids {
            let key = format!(
                "effect:{}:{ordinal}:{}:{source_range:?}:{:?}",
                registered.effect.as_str(),
                fact.kind.as_str(),
                fact.effect_group.map(|group| group.id),
            );
            let body = bodies
                .get_mut(&body_id)
