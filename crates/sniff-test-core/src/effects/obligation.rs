...
pub struct ContractId(AnnotationId);
...
impl ObligationState {
    ...
    #[must_use]
    pub fn remaining(&self) -> impl ExactSizeIterator<Item = usize> + '_ {
        self.remaining.iter().map(|obligation| obligation.index)
    }

    #[must_use]
    pub const fn effect(&self) -> &EffectKey {
        &self.effect
    }
    ...
    #[must_use]
    pub const fn source_invocation(&self) -> Option<InvocationId> {
        self.source_invocation
    }

    #[must_use]
    pub fn source_calls(&self) -> impl ExactSizeIterator<Item = CallId> + '_ {
        self.source_calls.iter().copied()
    }
}

#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord)]
pub struct ObligationMarkerUse {
...
impl ObligationMarkerUse {
    ...
    #[must_use]
    pub const fn source_invocation(&self) -> InvocationId {
        self.source_invocation
    }
    ...
    #[must_use]
    pub const fn node(&self) -> TraceNodeId {
        self.node
    }
}
...
impl ObligationContract {
    ...
    #[must_use]
    pub fn applies_to(&self, function: FunctionId) -> bool {
...
pub struct ObligationTracker<'annotations> {
...
pub struct ObligationEffectPolicy {
    effect: EffectKey,
...
impl<'annotations> ObligationTracker<'annotations> {
    ...
    #[must_use]
    pub fn contract(&self, id: ContractId) -> Option<&ObligationContract> {
    ...
    #[must_use]
    #[cfg(test)]
    pub fn contract_count(&self, effect: &EffectKey) -> usize {
        self.contracts
            .iter()
    ...
    #[must_use]
    pub fn trusts_function(&self, effect: &EffectKey, function: FunctionId) -> bool {
        self.policies
            .get(effect)
    ...
    #[must_use]
    pub fn is_ignored_invocation(&self, effect: &EffectKey, invocation: InvocationId) -> bool {
        self.policies
            .get(effect)
...
impl<S> TrackedState<S> {
    #[must_use]
    pub const fn obligation(&self) -> Option<&ObligationState> {
        match self {
...
impl<'effect, 'annotations, C> TrackedEffect<'effect, 'annotations, C> {
    ...
    #[must_use]
    pub fn obligation_marker_uses<O: Clone, S, T>(
        &self,
