...
fn effect_selection_uses_registered_keys_generically() {
    ...
    assert_eq!(
        all.fingerprint(&super::EffectSelection::registered_keys()),
        explicit_all.fingerprint(&super::EffectSelection::registered_keys())
    );
    assert_ne!(
        all.fingerprint(&super::EffectSelection::registered_keys()),
        safety.fingerprint(&super::EffectSelection::registered_keys())
    );
}
...
fn grouped_effect_sources_are_terminated_per_raw_branch() {
    ...
    let panic_config = crate::config::test_config();
    let panic = probe_panic(
        &panic_artifact,
        &panic_graph,
        &panic_annotations,
...
fn disabling_unsafe_precondition_ignore_exports_internal_panic_contracts() {
    ...
    let config = crate::config::test_from_manifest_str("[panic]\nignored-namespaces = []\n")
