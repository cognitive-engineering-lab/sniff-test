...
impl<'de> Deserialize<'de> for SniffTestConfig {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_raw(raw, BTreeMap::new()).map_err(serde::de::Error::custom)
    }
}

impl SniffTestConfig {
    fn from_raw(
        raw: RawSniffTestConfig,
        effect_defaults: BTreeMap<String, EffectConfig>,
    ) -> Result<Self, String> {
        let mut config = Self {
        ...
        for (name, effect) in raw.effects {
            let Some(default) = config.effects.get_mut(&name) else {
                return Err(format!("unknown effect section `{name}`"));
            };
    ...
    pub fn from_manifest_path(path: impl AsRef<Path>) -> Result<Self, ConfigError> {
        Self::from_manifest_path_with_effects(path, BTreeMap::new())
    }

    pub fn from_manifest_path_with_effects(
        ...
        let mut config =
            Self::from_manifest_str_with_effects(&source, effect_defaults).map_err(|source| {
                ConfigError::Parse {
                    path: path.to_owned(),
                    source,
                }
    ...
    pub fn from_manifest_str(source: &str) -> Result<Self, toml::de::Error> {
        Self::from_manifest_str_with_effects(source, BTreeMap::new())
    }

    pub fn from_manifest_str_with_effects(
        source: &str,
        effect_defaults: BTreeMap<String, EffectConfig>,
    ) -> Result<Self, toml::de::Error> {
        let raw: RawSniffTestConfig = toml::from_str(source)?;
        Self::from_raw(raw, effect_defaults).map_err(serde::de::Error::custom)
    }

    pub fn emit_warnings(&self, path: &Path) {
...
#[cfg(test)]
#[must_use]
pub fn test_config() -> SniffTestConfig {
    SniffTestConfig {
...
#[cfg(test)]
pub fn test_from_manifest_str(source: &str) -> Result<SniffTestConfig, toml::de::Error> {
    SniffTestConfig::from_manifest_str_with_effects(
        source,
        crate::effects::registered_effect_configs(),
    )
}

#[cfg(test)]
pub fn test_from_manifest_path(path: impl AsRef<Path>) -> Result<SniffTestConfig, ConfigError> {
...
impl EffectConfig {
    ...
    #[must_use]
    pub fn ignored_namespaces(&self) -> &PathPatterns {
        &self.ignored_namespaces
    }

    #[must_use]
    pub fn trusted_boundary_namespaces(&self) -> &PathPatterns {
        &self.trusted_boundary_namespaces
    }

    #[must_use]
    pub fn finding_lints(&self) -> EffectFindingLints {
        EffectFindingLints {
    ...
    #[must_use]
    pub fn operation_lint(&self, operation: Option<&str>) -> LintLevel {
        operation
    ...
    #[must_use]
    pub fn effective_coverage(&self) -> EffectiveCoverageConfig {
    ...
    #[must_use]
    #[cfg(test)]
    pub fn ignores_candidates(&self, candidates: &[String]) -> bool {
        self.ignored_namespaces
...
pub struct EffectiveCoverageConfig {
    pub unresolved_call_target: LintLevel,
    pub analysis_incomplete: LintLevel,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct EffectFindingLints {
    pub concrete_invocation: LintLevel,
    pub ambiguous_marker: LintLevel,
    pub ambiguous_requirement: LintLevel,
}
...
#[allow(
    ...
    reason = "raw fields are consumed after warning collection"
)]
...
mod tests {
    ...
    fn manifest_warns_on_unknown_effect_fields_and_rejects_other_unknown_fields() {
        for (manifest, field) in [
            ...
            if manifest.starts_with("[panic") || manifest.starts_with("[safety") {
                ...
                let error = crate::config::test_from_manifest_str(manifest)
    ...
    fn parses_contracts_and_domain_local_trusted_boundary_namespaces() {
        ...
        let parsed = crate::config::test_from_manifest_str(config).expect("manifest should parse");
    ...
    fn overflow_checks_and_overflow_lint_parse_independently() {
        for (overflow, expected_overflow) in [
            ...
            let parsed = crate::config::test_from_manifest_str(&config)
    ...
    fn analysis_lints_reject_effect_specific_keys() {
        for key in [
            ...
            let error = crate::config::test_from_manifest_str(&source)
    ...
    fn unsafe_precondition_macro_is_a_user_overridable_default_ignore() {
        let defaults = crate::config::test_config().effect("panic").clone();
        assert!(
            defaults
                .ignored_namespaces
        ...
        let disabled = crate::config::test_from_manifest_str("[panic]\nignored-namespaces = []\n")
        ...
        let replacement = crate::config::test_from_manifest_str(
    ...
    fn example_manifest_trusts_standard_library_crates_while_empty_config_does_not() {
        let initialized = crate::config::test_from_manifest_str(EXAMPLE_MANIFEST)
        ...
        let empty = crate::config::test_config();
    ...
    fn default_safety_lints_keep_findings_visible_without_failing() {
        let lints = crate::config::test_config().effect("safety").lints.clone();
    ...
    fn operation_lint_tables_warn_on_unknown_and_flat_keys() {
        for source in [
            ...
            let config = crate::config::test_from_manifest_str(source)
                .expect("unknown fields produce warnings");
    ...
    fn operation_defaults_serialize_with_overrides() {
        let mut panic_lints = crate::config::test_config().effect("panic").lints.clone();
        panic_lints
            .operations
        ...
        let mut safety_lints = crate::config::test_config().effect("safety").lints.clone();
        safety_lints
            .operations
        ...
        assert!(
            toml::to_string(&crate::config::test_config().effect("panic").lints)
    ...
    fn panic_namespace_policies_use_stable_candidate_sets() {
        let config = EffectConfig {
            ...
            ..crate::config::test_config().effect("panic").clone()
        };
    ...
    fn example_manifest_preserves_ignored_macro_paths() {
        let config = crate::config::test_from_manifest_str(EXAMPLE_MANIFEST)
    ...
    fn manifest_validates_namespace_globs() {
        ...
        let error = crate::config::test_from_manifest_str(config)
    ...
    fn parses_report_roots() {
        ...
        let parsed_all = crate::config::test_from_manifest_str(all).expect("manifest should parse");
        let parsed_explicit =
            crate::config::test_from_manifest_str(explicit).expect("manifest should parse");
        assert_eq!(
    ...
    fn report_roots_retain_the_entire_config_value_span() {
        for (manifest, expected) in [
            ...
            let parsed =
                crate::config::test_from_manifest_str(manifest).expect("manifest should parse");
    ...
    fn manifest_loads_contract_overrides_relative_to_itself() {
        ...
        let parsed =
            crate::config::test_from_manifest_path(&manifest_path).expect("manifest should load");
