...
pub(crate) struct TrustPath(BTreeSet<u64>);

impl TrustPath {
    pub(crate) fn new(graph: &InvocationGraph, source: FunctionId) -> Self {
        Self(BTreeSet::from([Self::crate_id(graph, source)]))
    }

    pub(crate) fn enter(&mut self, graph: &InvocationGraph, function: FunctionId) {
        self.0.insert(Self::crate_id(graph, function));
    }

    pub(crate) fn allows_boundary(&self, graph: &InvocationGraph, boundary: FunctionId) -> bool {
