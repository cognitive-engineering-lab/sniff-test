...
checks does not change or reject `[panic.lints.operations].overflow`.
...
`[panic.lints]` and `[safety.lints]` set `ambiguous-marker` and
...
Exact keys under `[panic.lints.operations]` override `[panic.lints].operation`
...
An unmet documented obligation uses the effect's `[panic.lints].invocation`
...
Use `[panic].trusted-boundary-namespaces` for APIs whose caller-visible panic
...
`[panic].ignored-namespaces` matches both definition namespaces and macro
...
`[panic.lints] ambiguous-requirement = "deny"` or
