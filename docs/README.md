# sniff-test documentation

This document contains advanced usage, implementation details, output formats,
and contributor workflows. For the normal user workflow, start with the
...
contract is authoritative. Matching implementations are trusted, as are their
transitive dependencies on paths through the trusted API. Dependency trust is
not global: calling the same dependency directly from application code still
requires analysis. Effects from application-supplied callbacks or trait
implementations cross undocumented trusted APIs. An intervening `# Panics`
contract discharges the incoming effect and introduces its documented
obligations at callers, where they must be justified as usual. A trusted API's
own `# Panics` contract also remains visible to non-trusted callers.

Trust does not erase an untrusted callback from the path when that callback
calls back into a dependency of the trusted crate. Both raw effects and
undischarged documentation obligations retain this path context. The manifest
generated by `cargo sniff-test init` explicitly trusts `core`, `alloc`, and
`std`; remove an entry—or use `[]`—to audit those implementation internals.
...
implementation do not propagate out of it. Dependency trust is scoped to paths
through that implementation, as in the panic domain. Effects and obligations
from untrusted callbacks cross undocumented trusted APIs; a matching `# Safety`
contract discharges incoming effects and exports documented obligations. The matched API's own
